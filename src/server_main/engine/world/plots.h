...
{
    class Plots final : public shared::ConsumeDataProvider
    {
    public:
        Plots() = default;
        ...
        [[nodiscard]]
        bool Load(const std::filesystem::path& worldName) noexcept;

        [[nodiscard]]
        uint16_t GetPlotIndexByName(std::string_view name) const noexcept;

        [[nodiscard]]
        const std::shared_ptr<Plot>& GetPlot(uint16_t index) const noexcept
        {
